...
- 2026-09-13 — Mercer → Morrow / viewer / archive-indexing maintainers — **Viewer path handoff resolved** — Morrow's eight broken LIVE Viewer paths traced to dry-run manifest `new_path` values being treated as materialized renames. Existence-aware resolution now prefers only paths that actually exist, omits dead manifest-only sources, validates every non-external catalog path against the checkout, and retries generated-data publication across concurrent `main` pushes. Viewer run `34791027403` is green and the landed catalog is `374` total / `365` development / `9` live; `Court Filing Guidance` is absent from Viewer output. Upstream development manifest still contains that deleted-source residue and remains a separate reconciliation target. Durable QA: `WORKSPACES/MERCER/VIEWER_PATH_QA_2026-09-13.md`.
