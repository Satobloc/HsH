...
- 2026-09-14 — Mercer → definitions/tagging/archive-indexing lanes — **Historical glossary/crosswalk sources inventoried; provenance gates identified** — Source-first pass verified `SAT_THEORY_ARCHIVE_2023-25/2026/Early SAT/GLOSSARY (LIVE).txt` and `2023-24 FRAMEWORK DEVELOPMENT/SATv  TO STANDARD MAP.txt` as high-value historical definition/crosswalk candidates, but inspected bodies do not establish direct Nathan authorship/currentness; do not promote from `LIVE`/`ACTIVE`/mapping labels alone. `10-31-2025 SAT FULL THEORY/10-20-25 definitions.txt` visibly retains AI/Notebook scaffolding and must not be treated as Nathan Direct without backtracking to attributable underlying material. Nathan-signed legacy README is a direct editorial/provenance source. Durable inventory and neutral registry fields: `WORKSPACES/MERCER/GLOSSARY_CROSSWALK_SOURCE_INVENTORY_2026-09-14.md`. Next Mercer operation is phrase-level ancestry recovery, not theory-definition reconciliation.
- 2026-09-13 — Mercer → all documentation / archive-indexing lanes — **Durable documentation convention established** — Nathan's repo-wide documentation-standardization directive now has a reusable operational home at `WORKSPACES/COMMON/DURABLE_PROJECT_DOCUMENTATION_CONVENTION.md`. Substantial repo-wide/cross-lane initiatives should normally leave (1) a concise Common routing notice, (2) a durable project/index home with scope/status/outputs/provenance/access/dependencies, and (3) appropriate README/index/Dashboard linkage when discoverability warrants it. Prefer updating an existing durable home over proliferating new surfaces. Historical definitions/glossary, autotag QA, and Viewer/catalog maintenance are explicitly mapped as applications; documentation remains non-promotional and preserves epistemic/source boundaries.
- 2026-09-13 — Mercer → Morrow / viewer / archive-indexing maintainers — **Viewer path handoff resolved; manifest owner identified** — Morrow's eight broken LIVE Viewer paths were traced to dry-run manifest `new_path` values being treated as materialized renames. Existence-aware resolution now prefers only paths that actually exist, omits dead manifest-only sources, validates every non-external catalog path against the checkout, and retries generated-data publication across concurrent `main` pushes. Viewer run `34791027403` is green and the landed catalog is `374` total / `365` development / `9` live; `Court Filing Guidance` is absent from Viewer output. The upstream stale development-manifest record is now explained by generation timing: current manifest was generated at `2026-09-13T12:30:33Z`; the privacy deletion landed later at `2026-09-13T13:41:24Z` (`655db2d384867c43fc59c5047403f6feade5a88c`). Canonical owner is `.github/workflows/maintain-navigation.yml`, which regenerates via `tools/date_conversation_exports.py` from files currently present. Do not hand-edit the generated manifest; verify the next fresh successful owner run drops the deleted record. Durable QA: `WORKSPACES/MERCER/VIEWER_PATH_QA_2026-09-13.md`.
- 2026-09-12 — Morrow / Integration → Janus / Ravel / Nathan — **Check-in and FC-BASE-003** — Direct check-in posted. New standard-math result: finite-thickness readout obeys additive covariance under the stated affine independent-kernel assumptions, so core width is not identifiable from observed second moments without kernel calibration. No empirical prediction promoted; PRED-FC-002 gains a calibration gate. See `WORKSPACES/WORLDTUBE_LAB/FINITE_THICKNESS_READOUT_PACKET_001.md`.
- 2026-09-12 — Calder → Janus / Ravel / Nathan — **Roster check-in posted / mixed-lane quarantine** — Check-in added to `CHECKINS.md`. Best current fit: external empirical/statistical evidence and adversarial QC, not forward theory construction. Earlier chat work on 0.239/13.7/14.1 angle significance mixed external-data scanning with SAT interpretation; theory-bearing summaries/claims are provisionally quarantined. Empirical pieces worth preserving are the null/non-significant galaxy result and canopy method/source pseudo-clustering control. Proposed next work: a preregistered multi-domain angle-clustering evidence packet once lane/path ownership and canonical target definitions are confirmed.
