...
The current routing model is governed by the live Common control plane, especially:

- `WORKFLOW_BRANCHING_MAP.md`
...
Historical direction-unification and Dashboard materials remain provenance/strategic-intent evidence; current branch, milestone, and directive state controls execution.
...
Only five scheduled tasks may be active under the current scheduler constraint. Releasing a lease does not retire an instance; unscheduled and paused instances remain part of the pool where their recorded state permits reentry.

## Generalist / lease model

All enabled loops should read current central branch/task state before selecting work. Historical specialties are soft biases, not jurisdictions. A recurrence label does not require continuing its previous task when a different safe bounded operation is clearly higher value.

Tasks and branches may be split, merged, parked, resumed, reassigned, superseded, or retired under the current branching-map rules. Branch reassignment does not change instance identity, and instance lease reassignment does not require renaming any conversation.
...
- reassign or release execution leases among eligible instances;
- change priorities and scheduler-capacity allocation within the five-slot limit;
...
## Check-ins, continuity, and reassignment

Missing expected check-ins, stale checkpoints, automation failures, and unresolved blockers are workflow signals, not automatic failure findings. Diagnose first. When reassignment is warranted, preserve a continuity packet containing the last durable state, covered sources/artifacts, blocker or failed approaches, next cursor, exposure/quarantine state, exit criterion, and return route.

Paused is not abandoned. Unscheduled is not retired.

## Current hard boundaries

Current hard boundaries include:

- FIE fidelity as the governing SAT conceptual/methodological anchor;
- sandbox discipline for theory-bearing construction/reconstruction;
- quarantine adherence and PRIOR_ART/private-quarantine separation;
- evidence discipline and honest source/exposure history;
- current onboarding/reorientation requirements when triggered;
- current Common write-safety and authorship controls.

## Transitioned / unscheduled functions
...
- `WORKFLOW_BRANCHING_MAP.md` is the central routing map.
...
This roster is operational and deliberately revisable as branch priorities, milestones, instance availability, and lease fit change.