...
manifests, and its declared registered-external registry. It does not open raw
conversation bodies or descend repository trees.
"""
...
BLOCKED_STATUSES = {"skipped", "collision", "blocked"}
...
def manifest_items(payload: dict[str, Any], corpus: str, source: str) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for index, rec in enumerate(payload.get("records", [])):
        if not isinstance(rec, dict):
            continue
        candidate = rec.get("new_path") or rec.get("old_path")
        count = rec.get("message_count")
        if (not isinstance(candidate, str)
                or Path(candidate).suffix.lower() not in SUPPORTED_SUFFIXES
                or not isinstance(count, int) or count <= 0
                or str(rec.get("status") or "") in BLOCKED_STATUSES):
            continue
        out.append({
...
def diagnose(root: Path) -> dict[str, Any]:
    ...
    for inp in inputs:
        ...
        if corpus == "registered-external":
            ...
            accepted.extend(manifest_items(payload, str(corpus), rel))
