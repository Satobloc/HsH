...
**Current through:** Run 94, 2026-09-17
...
Run 93 traced a separate external-source interface constraint in Viewer curation. Whole-conversation hiding is source-location independent, but partial curation (`omit_ranges` / `only_ranges`) calls `write_curated_copy()`, which resolves `Path(item["path"])` and requires the source file in the local HsH checkout. External Viewer records deliberately point to canonical other-repository paths and are not copied into HsH. Therefore a canonical external-only conversation cannot currently receive partial curation through this builder path. `CURATION.json` is presently empty, so classify as a **latent capability/documentation mismatch**, not a current production failure. Keep distinct from the immutable-identity gap.

Run 94 found a useful counterexample/implementation precedent: `tools/build_viewer_body_search.py` already resolves canonical GLASS sources through a supplied external checkout/root and indexes curated copies separately. But the body-search subsystem is presently **dormant/unwired**: current committed Viewer data has no `data/body_search/`, `build-conversation-viewer.yml` does not invoke/publish the generator, and `index.html` does not load `catalog_body_search.js`. Current README promises within-conversation search, not cross-conversation body search, so this is not presently a demonstrated production regression or documentation breach. Do not activate it without interface review; retain its external-source resolution pattern as precedent for future Viewer design.
...
- `EXTERNAL VIEWER PARTIAL CURATION`: current builder requires partial-curation source bytes at local HsH `item[path]`; canonical external-only records therefore cannot use partial curation. Current rules are empty, so no production failure established. Route as adjacent Viewer-interface evidence if Sable review activates; do not duplicate the open request.
- `DORMANT VIEWER BODY SEARCH`: generator/browser module exist but are not wired into current workflow/page; generator's repository-aware GLASS resolution is a useful implementation precedent. No activation requested; do not treat dormancy as failure without intent/history evidence.
...
## Run history
Runs 1–7 training/scanner; 8–12 Viewer navigation QA; 13 documentation convention; 14–22 glossary/crosswalk provenance; 23–26 docs reconciliation; 27 corpus counts; 28 external semantics; 29 duplicate collision; 30–37 integrity validator/harness; 38–46 Viewer acceptance/dedup diagnosis; 47–51 semantics-aware validator transition; 52–53 autotag lineage audit/handoff; 54–69 conversation-identity diagnostics; 70 Viewer exposure audit; 71 sidecar specification; 72–73 synthetic harness; 74 continuity + Sable handoff repair; 75 Nathan Direct lineage re-audit; 76 normalization current-state reclassification; 77–80 routing/front-door currentness; 81–85 Dashboard/operational-currentness micro-audits; 86–89 formalization access/history/signature/branch checks; 90 external Viewer provenance-field comparison; 91 merge implementation trace; 92 manual-catalog production-scope check; 93 external partial-curation source-path trace; **94 audited cross-conversation body-search wiring, classified the subsystem as dormant/unwired rather than a production regression, and recovered its repository-aware GLASS resolution as a useful external-source processing precedent.**

## Best next operations
1. Inspect one other currently wired Viewer retrieval/navigation feature for source-location or generated-state assumptions, or rotate to extraction/sample QA if no higher-value Viewer target appears.
2. Inspect Sable inbox on future runs; if Run-71 review activates, supply immutable identity, partial-curation source resolution, and the dormant body-index builder's external-root pattern as distinct adjacent design evidence rather than opening duplicate requests.
...
6. Do not spend further cycles probing the missing Lean wrapper or dormant body-search activation without new evidence/intent.
