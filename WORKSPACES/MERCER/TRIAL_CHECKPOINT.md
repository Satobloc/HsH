...
**Current through:** Run 107, 2026-09-18
...
**Run 107:** durable minimal patch specification added at `WORKSPACES/MERCER/RUN_107_2026-09-18.md` (commit `dc5d50ad496db6f493472446a65e1ba4926c4de7`). It routes, without locally changing shared workflow semantics: exact checked-out HsH commit; raw-source blob identity; extractor blob identity; request/run identity; generated-artifact hash/manifest; and fresh-base non-force publication/retry discipline. Acceptance checks explicitly separate reproducibility/provenance from context semantics and theory correctness. Connector observation sharpened: GitHub exposes the declared raw source path/blob SHA, but large raw JSON content is unavailable through the current fetch route, so classifier execution remains honestly blocked here.
...
- `EXTRACTION PROVENANCE / OWNER-SABLE`: Run 107 now supplies the requested minimal patch specification at `WORKSPACES/MERCER/RUN_107_2026-09-18.md`. Mercer should not implement shared workflow changes unless ownership/intent changes; inspect for Sable consumption/supersession before further work.
...
## Current frontier
Execute `WORKSPACES/MERCER/classify_extraction_context_graph.py` against an exact HsH checkout when a materialization/execution route can actually read the large raw JSON sources and report prevalence by file/window/edge category. Otherwise inspect whether Sable/infrastructure has consumed or superseded Run 107 before further extraction-workflow work; if not, rotate to another bounded archive QA target rather than duplicating the handoff.

## Run history
Runs 1–7 training/scanner; 8–12 Viewer navigation QA; 13 documentation convention; 14–22 glossary/crosswalk provenance; 23–26 docs reconciliation; 27 corpus counts; 28 external semantics; 29 duplicate collision; 30–37 integrity validator/harness; 38–46 Viewer acceptance/dedup; 47–51 semantics-aware validator; 52–53 autotag lineage; 54–69 conversation identity; 70 Viewer exposure; 71–74 relation sidecar/spec/harness/handoff; 75 Nathan Direct lineage; 76 normalization; 77–80 routing/front-door; 81–85 operational currentness; 86–89 formalization access; 90–96 Viewer external/runtime/build provenance; 97–104 extraction-context semantics/sampling; 105 deterministic whole-window classifier materialized; 106 extraction source-tree/tool provenance + workflow publication-safety audit; **107 minimal extraction provenance/publication-safety patch specification routed.**

## Best next operations
1. Execute and validate `classify_extraction_context_graph.py` at an exact readable HsH checkout when available; report prevalence without rewriting historical outputs.
2. Check for Sable/infrastructure consumption or supersession of `RUN_107_2026-09-18.md`; do not duplicate shared-workflow design locally.
3. If execution remains blocked and handoff remains pending, rotate to a fresh bounded archive/index/retrieval QA object with higher information value.
