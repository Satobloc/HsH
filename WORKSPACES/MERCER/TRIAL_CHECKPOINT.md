...
**Current through:** Run 87, 2026-09-17
...
Runs 86–87: Dashboard `Equation / formalization = ACTIVE CAPABILITY TEST / LOW COVERAGE` routes to the live HsH `formalization/` programme, whose README advertises `formalization/leancheck/run.ps1` and `formalization/leancheck/README.md`. The path is absent on current `main`. History trace shows commit `c4e6bfa6e848a165ee2c1761b1c85a5328cba2af` (`Add Windows Lean check wrapper`) changed only `formalization/README.md`; direct lookup of the script at that commit is 404 and path-specific commit history for `formalization/leancheck/run.ps1` is empty. Strongest current classification: **documented-but-unmaterialized wrapper path**, not demonstrated later deletion. This does not exclude a local/unmerged/alternate-path copy and does not establish Lean execution or mathematical/model correctness. Durable records: Runs 86–87.
...
- `FORMALIZATION ACCESS`: advertised `formalization/leancheck/*` is now traced to a 2026-09-14 README-only commit; no commit history exists at `formalization/leancheck/run.ps1`. Search distinctive wrapper strings/alternate paths or branches before deciding whether to materialize tooling or correct documentation.
...
## Run history
Runs 1–7 training/scanner; 8–12 Viewer navigation QA; 13 documentation convention; 14–22 glossary/crosswalk provenance; 23–26 docs reconciliation; 27 corpus counts; 28 external semantics; 29 duplicate collision; 30–37 integrity validator/harness; 38–46 Viewer acceptance/dedup diagnosis; 47–51 semantics-aware validator transition; 52–53 autotag lineage audit/handoff; 54–69 conversation-identity diagnostics through production v4/all-family ancestry classification; 70 Viewer exposure audit; 71 sidecar specification; 72–73 synthetic harness; 74 continuity + Sable handoff repair; 75 Nathan Direct lineage re-audit; 76 normalization current-state reclassification; 77–80 routing/front-door currentness audits; 81 Dashboard Sable-programme micro-audit; 82 Sable inbox/currentness audit; 83 Dashboard-linked heartbeat audit; 84 live-scheduler comparison; 85 revival-rotation status-granularity audit; 86 formalization access-path audit; **87 formalization history trace: wrapper documentation was introduced in a README-only commit and no `formalization/leancheck/run.ps1` path history is recorded.**

## Best next operations
1. Search HsH current/history for distinctive wrapper strings (`SkipCache`, `RefreshMathlib`, `.work`, `lake env lean`) and alternate PowerShell paths before proposing formalization repair.
2. Inspect Sable response/inbox state for the Run-71 Viewer relation-sidecar proposal; do not implement shared schema unilaterally.
