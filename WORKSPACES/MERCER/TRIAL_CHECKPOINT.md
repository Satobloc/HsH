...
## Current frontier — after Run 18

### A. Historical glossary / standard-crosswalk provenance

**Status: source inventory + repository-custody layer complete; raw-message/content ancestry unresolved; Viewer catalog now verified as candidate-path/date bridge; three plausible candidates eliminated by bounded inspection.**
...
Run 16 exhausted the currently available GitHub indexed phrase-search route for exact/distinctive standard-map phrases. Do not repeat unless the searchable corpus/index changes.
...
Run 18 verified `CONVERSATION_VIEWER/catalog/conversations.json` as the missing **candidate-selection bridge**: it maps titles and dates to explicit raw repository paths, start/end times and message counts. Catalog proximity is retrieval metadata only, not authorship evidence.

Bounded Run-18 candidate results:

- `SAT theory overview`, catalog ID `fa2cc1b6a832`, 2025-10-29, 10 messages — no `SATv` reference or crosswalk-document creation/discussion signal; eliminated as a direct ancestry anchor.
- `Vetting document review`, raw conversation ID `69023bdb-63f0-832b-8821-56362ed2493c`, 2025-10-29, 16 catalog messages — inspection shows journal-submission vetting of attached `SPACE-S-25-00569.pdf`; no `SATv` / `standard map` match; eliminated for standard-map ancestry.
...
Current ancestry blocker is therefore **content-level anchor discovery**, not missing path/index/extraction machinery. Continue small, title/date-plausible candidate triage; if a direct artifact mention/paste appears, then use the bounded extractor to preserve exact Nathan message IDs/timestamps and relation.
...
Established repair chain remains in `WORKSPACES/MERCER/VIEWER_PATH_QA_2026-09-13.md`. The development manifest/catalog inspected in this run still reflects the older 2026-09-13 source state; this does not affect use of already-existing raw paths as provenance candidates.
...
- continued bounded catalog-driven ancestry triage for glossary/crosswalk sources;
...
- `DEPENDENCY`: direct raw-message ancestry for `GLOSSARY (LIVE).txt` and `SATv  TO STANDARD MAP.txt` remains unidentified. Tooling/path discovery are now available; missing piece is a content-level link.
...
- Run 18: identified Viewer catalog as a raw-path/date/message-count ancestry-selection index; bounded-inspected three high-signal candidates and eliminated them as direct glossary/crosswalk ancestry anchors without broad theory-content scanning. See `RUN_018_2026-09-14.md`.

## Best next operation

Next run: reread Control/Common. Do not poll terminal navigation run `34814002515`. For crosswalk ancestry, inspect only targeted ancestry anchors in the same-day `SATO-BLOC FULL LEANCHECK` raw conversation (larger, 121 messages) because its title provides the next strongest document-checking signal; do not summarize or synthesize its theory content. For glossary ancestry, select another small June-1 archival/document-management candidate from the Viewer catalog. If either directly names/pastes the target artifact, invoke the bounded raw extractor and record exact Nathan message/node IDs, timestamps, and relation. Otherwise record the bounded negative and branch rather than widening into speculative corpus scanning.
