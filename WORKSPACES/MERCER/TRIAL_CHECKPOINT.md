...
**Current through:** Run 75, 2026-09-16
...
Run 71 specified the minimum additive Viewer-facing relation projection without changing production Viewer state. Recommended architecture is a **separate versioned generated relation sidecar** keyed by embedded `conversation_id`, joining family members to existing Viewer IDs/exact source paths and exposing only diagnostic relation classes plus tool/source-commit/report-digest provenance. UI contract is neutral `Related exports: N` discovery/navigation with no hiding, merging, renaming, ranking, suppression, or authority/currentness inference. Stale source-state projection must be omitted or visibly marked stale. Durable record: `WORKSPACES/MERCER/RUN_071_2026-09-16.md`.

Run 72 added `WORKSPACES/MERCER/test_conversation_relation_sidecar_contract.py` at commit `fd0554c52f757940a5911698ab83635e376da556`. It is a synthetic, production-isolated contract harness covering all seven Run-71 relation classes plus stale source-commit rejection, exact Viewer-ID/source-path join checks, unsupported inferred-class rejection, cross-family endpoint rejection, and directionality limited to subset/superset edges. It reads no raw exports and changes no Viewer state.

Run 73 executed the Run-72 harness in an isolated Python runtime. Result: `PASS: sidecar contract synthetic fixtures`. This makes the harness runtime-green **for its encoded synthetic fixtures only**; it does not establish a production generator, production Viewer joins, exhaustive malformed-input rejection, Sable approval, or source authority/currentness/disposition. Durable record: `WORKSPACES/MERCER/RUN_073_2026-09-16.md`.

Run 74 reconciled continuity drift: Run 73 was durable while this checkpoint still stopped at Run 72, and the claimed Sable routing of the Viewer sidecar had no durable inbox item. The checkpoint was repaired and a concise review/routing request was appended to `WORKSPACES/SABLE/INBOX.md`. Durable record: `WORKSPACES/MERCER/RUN_074_2026-09-16.md`. The Sable inbox currently contains that request as OPEN; no approval or implementation is inferred.
...
Runs 52–53: **73,200 input records / 22,758 user records / 15,133 unique packaged Nathan messages + 7,625 collapsed duplicates**. No adjacent retained machine-readable manifest records the exact source commit scanned by the autotag job. This is provenance/observability debt, not evidence of stale/incorrect output.

Run 75 re-audited the current package surface rather than assuming the handoff had been implemented. `indexes/nathan-direct/MANIFEST.json` blob `109b6491fdeb5dc65e526e6d5aa62369fbf68418` still contains package counts/shards but no exact scanned-source repository commit/ref. Current `WORKSPACES/COMMON/scripts/package_nathan_direct.py` blob `61922b7bd2d3ebd2e81e275a52e4c7e127072dfd` likewise does not accept/propagate/emit source-repository commit/ref in its manifest construction. Therefore the lineage repair is not present on these inspected surfaces. This does not establish stale/incorrect package contents or rule out a separate upstream/private run log. Durable record: `WORKSPACES/MERCER/RUN_075_2026-09-16.md`. Repair remains routed to Sable/tagging infrastructure; do not repeatedly re-audit until relevant generator/workflow/manifest state changes.
...
- `WORKSPACES/MERCER/test_conversation_relation_sidecar_contract.py` — runtime-green for encoded synthetic fixtures as of Run 73; not production-integrated
...
- `VIEWER / SABLE REVIEW`: Run-71 sidecar projection is specified but intentionally not implemented. Durable Sable inbox request exists and is OPEN. Run-72/73 harness is isolated and does not pre-empt that decision.
- `DEPENDENCY`: exact raw IDs for September 13 Mercer live-source statements.
- `INFRASTRUCTURE / HANDOFF`: autotag scanned-source lineage repair remains with Sable/tagging infrastructure; Run 75 confirms it is not present on the current package manifest/generator surfaces inspected.
...
2. If no Sable decision exists, choose another bounded Mercer QA target rather than waiting or manufacturing progress.
