...
**Current through:** Run 90, 2026-09-17
...
Run 90 found a bounded external-source provenance asymmetry: the manually registered Srena record carries repository/branch/path/URLs/parser metadata but no content hash or immutable source commit/blob identity, and the generated merged external record does not enrich it. Structurally discovered public cross-repo entries do carry `source_sha256`. Direct GitHub fetch confirms current `Satobloc/SAT_THEORY_ARCHIVE_2023-25/SRENA SAT2.txt` at blob `fe01fc85b1874c1573843724f035ee292fd48961`; that identity is not stored in the registration. Classify as provenance-observability/reproducibility asymmetry, not source error. Next cursor: inspect generator intent before proposing repair.
...
- `EXTERNAL VIEWER PROVENANCE`: manual Srena registration lacks content hash/immutable source identity while discovered external entries carry `source_sha256`; inspect generator intent before proposing shared-schema change.
...
## Run history
Runs 1–7 training/scanner; 8–12 Viewer navigation QA; 13 documentation convention; 14–22 glossary/crosswalk provenance; 23–26 docs reconciliation; 27 corpus counts; 28 external semantics; 29 duplicate collision; 30–37 integrity validator/harness; 38–46 Viewer acceptance/dedup diagnosis; 47–51 semantics-aware validator transition; 52–53 autotag lineage audit/handoff; 54–69 conversation-identity diagnostics; 70 Viewer exposure audit; 71 sidecar specification; 72–73 synthetic harness; 74 continuity + Sable handoff repair; 75 Nathan Direct lineage re-audit; 76 normalization current-state reclassification; 77–80 routing/front-door currentness; 81–85 Dashboard/operational-currentness micro-audits; 86 formalization access-path audit; 87 formalization history trace; 88 distinctive-signature search; 89 visible-branch formalization check; **90 external Viewer provenance-field comparison: manual Srena registration lacks immutable content identity while structurally discovered entries carry `source_sha256`; current canonical source blob independently resolved.**

## Best next operations
1. Inspect the external-discovery generator/workflow to determine whether manual registrations are intentionally exempt from content-hash enrichment; do not change shared schema before intent is clear.
